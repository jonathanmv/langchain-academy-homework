...
# By setting interrupt_before, the graph execution is interrupted before the assistant node is executed.
...
while True:
    ...
    if user_input == "y":
        ...
        system_message = SystemMessage(content="The user wants to perform a new operation.")
        # This updates the state. In this case, because it uses add_messages, the new operation will be added to the end of the messages list.
        # Messages could be edited but specifying an id.
        graph.update_state(config, {"messages": [system_message, HumanMessage(content=user_input)]})
    elif user_input == "q":
        break
